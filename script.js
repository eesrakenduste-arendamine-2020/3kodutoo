...
function nightMode(){
    if($("#night").html() == "Öö"){
		$("#night").html("Päev");
		$("#calculator").css({"background-color":"grey"});
		$("body").css({"background-color":"black"});
        $(".number").css({"background-color":"white", "color":"teal"});
        $("#name").css({"color":"silver"});
        $("#clear").css({"background-color":"silver","color":"black"});
        $("#backspace").css({"background-color":"silver", "color":"black"});
        $("button:nth-child(20)").css({"background-color":"silver", "color":"black"});
...
repoContainer.addEventListener("click", ()=>{
    window.open("https://github.com/Raimooo/3kodutoo");
});
